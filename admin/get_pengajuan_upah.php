...
// [PERBAIKAN] Query Detail Pengajuan, ditambahkan volume_rab dan nama_satuan
// Query detail pengajuan upah
$sql_detail = "SELECT
               ...
               WHERE dp.id_pengajuan_upah = ?
               ORDER BY d.nomor_urut_kategori ASC, d.id_detail_rab_upah ASC";

// [PENTING] Pastikan kamu mengeksekusinya dengan Prepared Statement
            ...
            <!-- Tabel Detail Pekerjaan -->
                    <div class="card shadow-sm">
                        <div class="card-header bg-light d-flex justify-content-between align-items-center">
                            ...
                            <a href="cetak_formulir_pengajuan.php?id=<?= $id_pengajuan_upah ?>" target="_blank" class="btn btn-info btn-round btn-sm">
                                ...
                                <table class="table table-bordered table-vcenter mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th style="width:8%;" class="text-center">Volume</th>
                                    ...
                                    <tbody>
                                        <?php if ($detail_result && mysqli_num_rows($detail_result) > 0): ?>
                                            <?php $prevKategori = null; $noKategori = 0; ?>
                                            <?php while ($row = mysqli_fetch_assoc($detail_result)): ?>
                                                <?php 
                                                    if ($prevKategori !== $row['nama_kategori']) {
                                                        $noKategori++;
                                                        echo "<tr class='table-primary fw-bold'><td class='text-center'>" . toRoman($noKategori) . "</td><td colspan='6'>" . htmlspecialchars(strtoupper($row['nama_kategori'])) . "</td></tr>";
                                                ...
                                                <tr>
                                                    ...
                                                    <td class='text-end fw-bold'>Rp <?= number_format($row['nilai_upah_diajukan'], 0, ',', '.') ?></td>
                                                </tr>
                                            <?php endwhile; ?>
                                        <?php else: ?>
                                            <tr><td colspan="7" class="text-center p-4">Tidak ada rincian pekerjaan untuk pengajuan ini.</td></tr>
                                        <?php endif; ?>
                                    </tbody>
                                    <tfoot>
