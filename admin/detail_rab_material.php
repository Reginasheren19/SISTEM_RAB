...
$sql = "SELECT 
            ...
            mpr.type_proyek,
            mpe.lokasi,
            YEAR(tr.tanggal) AS tahun,
            mm.nama_mandor,
            tr.tanggal
        FROM transaksi_rab_material tr
...
// Buat formatted ID sesuai keinginan: "RABM{id}{tahun}"
$tahun = date('Y');  // Bisa juga dari $data['tahun'] jika mau sesuai tanggal RAB-nya
$formatted_id = 'RABM' . $data['id_rab_material'] . $tahun;

// Query ambil detail rab material (pekerjaan dan biaya)
$sql_detail = "SELECT d.id_detail_rab_material, mp.uraian_pekerjaan, d.volume, d.harga_satuan, d.total_rab_material
...
<html lang="en">
  <head>
    ...
    <title>Transaksi RAB Material</title>
    <meta
  ...
  <body>
        ...
        <tbody>
          ...
          if ($detail_result && mysqli_num_rows($detail_result) > 0) {
              ...
              while ($row = mysqli_fetch_assoc($detail_result)) {
                  $grand_total += $row['total_rab_material'];
                  echo "<tr>
                          <td>" . $no++ . "</td>
    ...
    <a href="transaksi_rab_material.php" class="btn btn-secondary">Kembali</a>
</div>
